"""Check descriptions for secfesc tools."""
...
CHECK_DESCRIPTIONS = {
    ...
    "open_ports": {
        ...
        "fix": "Review open ports with: ss -tulnp | Disable unused: sudo systemctl disable --now <service>",
    },
    "firewall_rules": {
    ...
    "services": {
        ...
        "bad": "Suspicious services (e.g. telnetd, rshd) or unnecessary services detected.",
        "fix": "List running services: systemctl list-units --type=service --state=running",
    },
    "tcp_syn_cookies": {
...
LOGO_FULL = """
    ╔══════════════════════════════════════╗
    ║  secfesc  v1.6.0                      ║
    ║  Linux Security Inspection Tool       ║
    ╚══════════════════════════════════════╝
"""
...
def print_help() -> None:
    ...
    print("  Usage")
    _divider()
    print("    secfetch              Quick security overview")
    print("    secfetch --short      Compact one-box summary")
    print("    secscan               Basic audit (no root)")
    print("    secscan --full        Complete audit (requires root)")
    print("    secscan --category    Specific category only")
    print("    secscan --report      Export results (json/html/csv)")
    print("    secfetch help         Show all checks")
    print()
...
def print_check_help(name: str) -> None:
    ...
    if not info:
        log_warning(f"Unknown check: '{name}'")
        print(f"\n  [!] Unknown check: '{name}'")
